...
// Update the page to use CMS data
import { getAllDesignedObjects } from "@/lib/contentful"

/**
 * Animation variants for page elements
 * fadeIn: Animates elements from slightly below their position to their final position with opacity
 * staggerContainer: Container animation that staggers the animations of its children
 * imageReveal: Specific animation for images that includes a subtle scale effect
 */
const fadeIn = {
 ...
 * Uses the Intersection Observer API via framer-motion's useInView hook.
 *
 * @param {React.ReactNode} children - The content to be animated
 * @param {number} delay - Optional delay before animation starts (in seconds)
...
/**
 * DesignedObjectsPage Component
 *
 * Main component for the Designed Objects page.
 * Displays a collection of designed objects with filtering capabilities.
 *
 * @returns {JSX.Element} - The complete Designed Objects page
 */
export default async function DesignedObjectsPage() {
  // Get data from Contentful at build time
  const designedObjects = await getAllDesignedObjects()
  // State to track the currently active category filter
  const [activeCategory, setActiveCategory] = useState<string>("all")

  // Filter objects based on the selected category
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-6">
      {/* Hero Section - Page title and accent bar */}
      ...
      {/* Introduction Section - Explains the philosophy behind the objects */}
      <AnimatedSection>
      ...
      {/* Category Filter Buttons - Allow users to filter objects by material */}
      <AnimatedSection>
      ...
      {/* Featured Object Section - Only shown when viewing all objects */}
      {activeCategory === "all" && (
        <AnimatedSection>
      ...
      {/* Objects Grid - Displays filtered objects in a responsive grid */}
      <AnimatedSection>
        <motion.div variants={fadeIn} className="mb-20">
...
// This ensures the page is statically generated at build time with the latest CMS data
